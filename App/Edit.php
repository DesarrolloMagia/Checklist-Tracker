...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql = "UPDATE tareas SET nombre = ?, descripcion = ?, fecha_compromiso = ?, prioridad = ?, responsable = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);

        if ($stmt->execute([$nombre, $descripcion, $fecha_compromiso, $prioridad, $responsable, $id])) {
            // Tarea actualizada con éxito, redirige de vuelta a la vista de tareas pendientes
